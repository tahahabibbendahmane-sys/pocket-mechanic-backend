  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, backgroundColor: c.background }]}>
      ...
      <View style={styles.headerRow}>
        <Text style={[TYPE.displayMD, { color: c.text }]}>{t.settings.title}</Text>
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.profileRow}>
            ...
            <View style={styles.profileText}>
              ...
              <View style={styles.xpBarWide}>
                <View style={[styles.xpBarTrackWide, { backgroundColor: isDark ? '#2A2A2A' : '#E5E5E5' }]}>
              ...
              {!isGuest ? (
                <TouchableOpacity activeOpacity={0.7} onPress={() => router.push('/edit-profile')}>
                  <Text style={[TYPE.bodySM, { color: COLORS.blue, marginTop: SPACING.sm }]}>{t.settings.editProfile} →</Text>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity activeOpacity={0.7} onPress={() => router.push('/login')}>
                  <Text style={[TYPE.bodySM, { color: COLORS.blue, marginTop: SPACING.sm }]}>{t.settings.signInToSync} →</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
        ...
        <View style={styles.statsRow}>
          <ChunkyCard style={styles.statMini} variant="blue">
            <Text style={styles.statEmoji}>🔥</Text>
            <Text style={[TYPE.statSM, { color: COLORS.streakOrange }]}>{String(streakCount ?? 0)}</Text>
            <Text style={[TYPE.labelSM, { color: c.textSecondary }]}>Streak</Text>
          </ChunkyCard>
          <ChunkyCard style={styles.statMini} variant="green">
            <Text style={styles.statEmoji}>⚡</Text>
            <Text style={[TYPE.statSM, { color: COLORS.xpGreen }]}>{String(totalXP ?? 0)}</Text>
            <Text style={[TYPE.labelSM, { color: c.textSecondary }]}>Total XP</Text>
          </ChunkyCard>
          <ChunkyCard style={styles.statMini} variant="purple">
            <Text style={styles.statEmoji}>🏆</Text>
            <Text style={[TYPE.statSM, { color: COLORS.levelPurple }]}>{String(badgeCount ?? 0)}</Text>
            <Text style={[TYPE.labelSM, { color: c.textSecondary }]}>Badges</Text>
          </ChunkyCard>
        </View>

        {/* Documents */}
        <Text style={[TYPE.labelSM, { color: c.textMuted, marginBottom: SPACING.sm, marginTop: SPACING.lg }]}>DOCUMENTS</Text>
        ...
        {/* Preferences */}
        <Text style={[TYPE.labelSM, { color: c.textMuted, marginBottom: SPACING.sm, marginTop: SPACING.lg }]}>{t.settings.preferences}</Text>
        <ChunkyCard noPadding style={styles.settingsGroup}>
          <SettingRow icon="globe-outline" label={t.settings.language} value={languageLabel} onPress={cycleLanguage} c={c} />
          <View style={[styles.divider, { backgroundColor: c.divider }]} />
          <SettingRow icon="speedometer-outline" label={t.settings.units} value={unitSystem === 'metric' ? t.settings.kilometers : t.settings.miles} onPress={toggleUnits} c={c} />
          <View style={[styles.divider, { backgroundColor: c.divider }]} />
          <SettingRow icon="moon-outline" label={t.settings.darkMode} c={c}
            rightElement={<Switch value={isDark} onValueChange={toggleTheme} trackColor={{ false: '#3A3A3A', true: COLORS.blue }} thumbColor="#FFFFFF" ios_backgroundColor="#3A3A3A" />}
          />
        </ChunkyCard>

        {/* Notifications */}
        <Text style={[TYPE.labelSM, { color: c.textMuted, marginBottom: SPACING.sm, marginTop: SPACING.lg }]}>{t.settings.notifications}</Text>
        <ChunkyCard noPadding style={styles.settingsGroup}>
          <SettingRow icon="notifications-outline" label={t.settings.maintenanceReminders} c={c}
            rightElement={<Switch value={notificationsEnabled} onValueChange={toggleNotifications} trackColor={{ false: '#3A3A3A', true: COLORS.blue }} thumbColor="#FFFFFF" ios_backgroundColor="#3A3A3A" />}
          />
          <View style={[styles.divider, { backgroundColor: c.divider }]} />
          <SettingRow icon="construct-outline" label={t.settings.serviceAlerts} c={c}
            rightElement={<Switch value={serviceAlertsEnabled} onValueChange={toggleServiceAlerts} trackColor={{ false: '#3A3A3A', true: COLORS.blue }} thumbColor="#FFFFFF" ios_backgroundColor="#3A3A3A" />}
          />
          <View style={[styles.divider, { backgroundColor: c.divider }]} />
          <SettingRow
            ...
            rightElement={
              <Switch
                ...
                trackColor={{ false: '#3A3A3A', true: COLORS.blue }}
                thumbColor="#FFFFFF"
        ...
        {/* About */}
        <Text style={[TYPE.labelSM, { color: c.textMuted, marginBottom: SPACING.sm, marginTop: SPACING.lg }]}>{t.settings.about}</Text>
        <ChunkyCard noPadding style={styles.settingsGroup}>
...
function SettingRow({ icon, label, value, onPress, rightElement, c }: {
  ...
  const inner = (
    <View style={styles.settingRow}>
      <View style={styles.settingRowLeft}>
        <Ionicons name={icon as any} size={20} color={COLORS.blue} />
        <Text style={[TYPE.body, { color: c.text }]}>{label}</Text>
      </View>
      {rightElement ?? (
        <View style={styles.settingRowRight}>
          {value ? <Text style={[TYPE.body, { color: c.textMuted }]}>{value}</Text> : null}
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 56, height: 56, borderRadius: 28, backgroundColor: COLORS.blue, justifyContent: 'center', alignItems: 'center' },
  avatarText: { ...TYPE.h1, color: '#000' },
  profileText: { flex: 1 },
  levelPill: { backgroundColor: COLORS.blue + '20', borderRadius: RADIUS.pill, paddingHorizontal: 12, paddingVertical: 5, alignSelf: 'flex-start', marginBottom: SPACING.sm },
  levelPillText: { ...TYPE.label, color: COLORS.blue },
  xpBarWide: { marginTop: 0 },
  xpBarTrackWide: { height: 8, borderRadius: 4, overflow: 'hidden' },
  xpBarFillWide: { height: '100%', backgroundColor: COLORS.blue, borderRadius: 4 },
  ...
  badge: { backgroundColor: COLORS.blue, paddingHorizontal: 8, paddingVertical: 3, borderRadius: RADIUS.pill },
  badgeText: { ...TYPE.labelSM, color: '#000' },
  ...
  logoutBtn: { borderWidth: 2.5, borderBottomWidth: 4, borderRadius: RADIUS.sm, paddingVertical: SPACING.md, alignItems: 'center' },
});
